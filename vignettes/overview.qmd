...
Most functions return `tidybreed_pop` invisibly, enabling pipe chains. `tidybreed_table` is the input to all action functions (`add_phenotype`, `add_tbv`, `add_genotypes`, `define_additive_effects`, `define_chip`, etc.) and supports `filter()`, `collect()`, `select()`, `arrange()`, `pull()`, and `count()` directly.
...
flowchart TB
    subgraph R["R Session (Control Layer)"]
        ...
        POP -->|"get_table()"| TBL
        TBL -->|"mutate_table()<br/>add_phenotype()<br/>add_tbv()<br/>define_additive_effects()<br/>define_chip()<br/>extract_genotypes()"| POP
    end
...
flowchart TD
    ...
    subgraph P1["Phase 1 — Initialize"]
        F1["initialize_genome()<br/>pop_name, n_loci, n_chr, chr_len_Mb<br/>+ define_founder_haplotypes()"]
    end
    ...
    subgraph P3["Phase 3 — Traits"]
        F3["define_trait()<br/>trait_name, trait_type,<br/>target_add_var, residual_var"]
        F3B["define_effect_fixed_class()<br/>define_effect_fixed_cov()<br/>define_effect_random()"]
        F3 --> F3B
    end

    subgraph P4["Phase 4 — Genetic Architecture"]
        F4["get_table('genome_meta') |><br/>  filter(chr %in% 1:5) |><br/>  define_additive_effects(trait_name)<br/>(scalar or vector for correlated traits)"]
    end
...
pop <- initialize_genome(
  ...
  define_founder_haplotypes(n_haplotypes = 1000)
