...
test_that("extract_genotypes() with a real quote-embedded locus_name still returns correct results", {
  ...
  # Rename one causal locus to embed a single quote, keeping genome_meta and
  # ind_haplotype consistent (locus_name is denormalized across both; the
  # effect tables store locus_id only, and genome_effect_loci joins the name
  # back in).
  for (tbl in c("genome_meta", "ind_haplotype")) {
    DBI::dbExecute(pop$db_conn, paste0(
      "UPDATE ", tbl, " SET locus_name = 'Locus_1''s' WHERE locus_name = 'Locus_1'"
    ))
  }

  # loci_tbl is the path that still builds an IN list out of locus names, so
  # it is the one sql_in_list() has to escape.
  geno <- pop |>
    get_table("ind_meta") |>
    extract_genotypes(
      loci_tbl = get_table(pop, "genome_meta") |>
        dplyr::filter(locus_id <= 3)
    )

  expect_equal(nrow(geno), 4L)
  locus_cols <- grep("^locus_", names(geno), value = TRUE)
  expect_equal(length(locus_cols), 3L)  # still all 3 loci present

  # The effects path resolves locus_id off genome_effect_loci, so a quote in
  # the name cannot reach the SQL text at all.
  geno_eff <- pop |>
    get_table("ind_meta") |>
    extract_genotypes(
  ...
  expect_equal(names(geno_eff), names(geno))
})
