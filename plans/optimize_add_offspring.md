...
| 2 | Pure-R reference generator, per-gamete `dqrng` streams, **log-accumulation** inversion Poisson sampler, long output, optional crossover buffer, seed API (intentional stream change) | dqrng | ✅ Done (2026-07-07) — **leaner path**: dqrng swap in place (kept per-parent matrices + Stage-1 reshape; packed arrays/group-by-map/codes deferred to Stage 3). `seed`/`store_crossovers` APIs, `ind_crossover` writes, BIGINT-safe id; parity golden regenerated; 1332 tests green. See [Stage-2 summary](optimize_add_offspring_stage_2_summary.md). |
| 3 | C++17 kernel (Rcpp + `dqrng.h`), one entry point, autosome-only, same log-accumulation Poisson, group-by-map calls, R↔C++ parity test (incl. crossovers + sex-chr wrapper parity) | Rcpp, dqrng, BH, sitmo | ✅ Done (2026-07-07) — split **3a** (output-neutral gamete-flat/packed/coded/long-native seam + group-by-map) then **3b** (`src/make_gametes.cpp`, runtime selector, R kept as oracle/fallback). Byte-identical R↔C++ parity (kernel + wrapper incl. sex chr); 1531 tests green; `R CMD check` compiled-code OK, 0 errors. Bump to 0.53.0. See [Stage-3 summary](optimize_add_offspring_stage_3_summary.md). |
