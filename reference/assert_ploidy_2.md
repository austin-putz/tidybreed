...
\> 2) is not yet supported. Sex-linked and organelle `chr_meta` rules
(`copy_mode_M`/`copy_mode_F`) are fine here — `SUM(allele)` over however
many `ind_haplotype` rows exist per locus already produces correct
dosage regardless of row count, so this guard targets actual organism
ploidy, not `chr_meta` shape.
