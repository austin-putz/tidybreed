...
locus_names_chr_1 <- pop |> get_table("genome_meta") |>
  filter(chr_name == "1") |>
  pull(locus_name)

# list X and Y chromosomes loci names
locus_names_chr_X <- pop |> get_table("genome_meta") |>
  filter(chr_name == "X") |>
  pull(locus_name)
locus_names_chr_Y <- pop |> get_table("genome_meta") |>
  filter(chr_name == "Y") |>
  pull(locus_name)
...
# print updated chr_meta table to fix the X/Y chromosomes
pop |> get_table("chr_meta")
...
# wrote to founder_haplotypes table

# print table with individual data in it
...
pop |> 
  get_table("ind_haplotype") |>
  filter(
    locus_name %in% locus_names_chr_1
  ) |>
  count(id_ind)
...
pop |> 
  get_table("ind_haplotype") |>
  filter(
    locus_name %in% locus_names_chr_X
  ) |>
  count(id_ind)
...
pop |> 
  get_table("ind_haplotype") |>
  filter(
    locus_name %in% locus_names_chr_Y
  ) |>
  count(id_ind)
