...
#'   `software = "blupf90"` or `parent_avg = TRUE` must be supplied — leaving
#'   both at their defaults, or setting both, is an error.
...
#'   with `software`; see `software` above. Default `FALSE`.
...
#'   candidates when building the pedigree file. Default `2`. Used only in
...
#'   (validated up front regardless of mode). Used only in
#'   `software = "blupf90"` mode; silently unused under `parent_avg = TRUE`.
#' @param run_dir Character. Base directory for evaluation sub-folders.
#'   Default `"."` (working directory). Ignored when `pop$run_dirs` is
#'   non-empty (i.e. when [open_pop()] was called with a `tools` argument) —
#'   the managed output folder structure is used instead. Applies only in
#'   `software = "blupf90"` mode.
#' @param eval_id Character or `NULL`. Sub-folder name suffix appended to
#'   `"eval_"`. Auto-generated from `model` + timestamp when `NULL`. Ignored
#'   in managed output mode (see `run_dir` above); the run directory name
#'   generated by `.create_run_dir()` is used as the identifier. Applies only
#'   in `software = "blupf90"` mode.
...
#'   Applies only in `software = "blupf90"` mode.
#' @param ... Optional extra columns written to `ind_ebv` (scalars only;
...
ebv_blupf90 <- function(pop, subset_ids, trait_name, model, eval_nums,
  ...
  # Validate managed output structure before the binary lookup so configuration
  # errors surface first (before a "binary not found" error).
  managed <- length(pop$run_dirs) > 0
  if (managed) {
    if (!"blupf90" %in% names(pop$run_dirs)) {
      registered_tools <- setdiff(names(pop$run_dirs), "base")
      stop(
        "The 'blupf90' tool directory is not set up in this scenario's output folder.\n",
        "Add \"blupf90\" to the tools argument when calling open_pop(), e.g.:\n",
        "  open_pop(..., tools = c(\"blupf90\"))\n",
        if (length(registered_tools) > 0)
          paste0("Currently registered tools: ", paste(registered_tools, collapse = ", "))
        else
    ...
    if (!is.null(eval_id))
      warning("`eval_id` is ignored when pop has a managed output folder structure.",
              call. = FALSE)
    if (!identical(run_dir, "."))
      warning("`run_dir` is ignored when pop has a managed output folder structure.",
              call. = FALSE)
  }
  ...
  if (managed) {
    eval_dir <- .create_run_dir(pop, "blupf90")
    eval_id  <- basename(eval_dir)
  } else {
    if (is.null(eval_id))
