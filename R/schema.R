...
.genome_descriptions <- function() {
  rbind(
    ...
    .sm_tbl("genome_effects",
            "Genome-effect terms. One row per term: a single coefficient over one or more loci. The loci are in genome_effect_members; scope (line, parent of origin) is in genome_effect_member_origins. A locus is causal for a trait if it appears as a member of any term."),
    ...
    .sm_col("genome_effects", "effect_owner",
            "Which writer owns these rows, for replacement. Owners always sum and are never selected between: 'generated_additive_tbv' is reserved for define_additive_effects(), 'custom' is the default for define_genome_effects()"),
    .sm_col("genome_effects", "effect_name",
            "Optional per-term label; carries no mathematical meaning"),
    .sm_col("genome_effects", "genome_value",
            "The term's coefficient, in trait units"),
    # genome_effect_members
    .sm_tbl("genome_effect_members",
            "The loci a term spans, one row per (term x locus), canonicalized by ascending locus_id. contrast_name says which basis function the locus contributes: 'additive' (per allele copy), 'dominance' (Cockerham, diploid loci), or 'indicator' (one genotype state)."),
    .sm_col("genome_effect_members", "id_genome_effect",
            "Term identifier, matching genome_effects.id_genome_effect (R-enforced by validate_genome_effects(); no SQL FK, see define_genome.R); part of the primary key"),
    .sm_col("genome_effect_members", "member_slot",
            "Position within the term, 1..n in ascending locus_id order; part of the primary key"),
    .sm_col("genome_effect_members", "locus_id",
            "Locus identifier; FK to genome_meta.locus_id"),
    .sm_col("genome_effect_members", "contrast_name",
            "Basis function at this locus: 'additive', 'dominance' or 'indicator'"),
    .sm_col("genome_effect_members", "copy_count_value",
            "Indicator state: realized number of copies at this locus. NULL for non-indicator contrasts. Required alongside dosage_value because dosage alone conflates 'no copy', 'one allele-0 copy' and 'two allele-0 copies'"),
    .sm_col("genome_effect_members", "dosage_value",
            "Indicator state: dosage of allele 1 at this locus. NULL for non-indicator contrasts"),
    .sm_col("genome_effect_members", "center_value",
            "Per-copy centring constant: the base allele frequency p under Cockerham coding, 0.5 under functional coding. Required for 'additive' and 'dominance'; NULL for 'indicator'"),
    # genome_effect_member_origins
    .sm_tbl("genome_effect_member_origins",
            "Scope of a member, as a predicate over allele copies. No rows = the common scope, which matches every copy. An additive member takes at most one row; a genotype member takes an exact multiset whose copy counts sum to the realized copy count. Among matching variants of one family the most specific wins; overlapping but incomparable scopes are refused at write time."),
    .sm_col("genome_effect_member_origins", "id_genome_effect",
            "Term identifier, matching genome_effect_members.id_genome_effect (R-enforced; no SQL FK); part of the primary key"),
    .sm_col("genome_effect_member_origins", "member_slot",
            "Member this scope applies to, matching genome_effect_members.member_slot (R-enforced; no SQL FK); part of the primary key"),
    .sm_col("genome_effect_member_origins", "origin_slot",
            "Position within the member's origin multiset, canonicalized by sorting (line_match_type, line_name, parent_origin, copy_count)"),
    .sm_col("genome_effect_member_origins", "line_match_type",
            "How line_origin is matched: 'exact' (named line), 'unknown' (copies whose founding line is NULL), or 'any' (any line; additive members only, and must carry a parent_origin)"),
    .sm_col("genome_effect_member_origins", "line_name",
            "Founding line matched, for line_match_type 'exact'; NULL otherwise"),
    .sm_col("genome_effect_member_origins", "parent_origin",
            "Parent of origin matched: 1 = sire, 2 = dam, NULL = either. A non-NULL value on an additive member is how imprinting is expressed"),
    .sm_col("genome_effect_member_origins", "copy_count",
            "Number of copies this row demands; always 1 on an additive member, where matching is per copy"),
    # genome_effect_terms (view)
    .sm_tbl("genome_effect_terms",
            "View: one row per term, with effect_order (member count) and family_key derived rather than stored. Terms sharing a family_key are scope variants of one mathematical term and compete under specificity fallback; terms with different keys sum."),
    .sm_col("genome_effect_terms", "trait_name",
            "Genetic component trait; from genome_effects"),
    .sm_col("genome_effect_terms", "effect_owner",
            "Owning writer; from genome_effects"),
    .sm_col("genome_effect_terms", "effect_name",
            "Optional per-term label; from genome_effects"),
    .sm_col("genome_effect_terms", "id_genome_effect",
            "Term identifier; from genome_effects"),
    .sm_col("genome_effect_terms", "genome_value",
            "The term's coefficient; from genome_effects"),
    .sm_col("genome_effect_terms", "effect_order",
            "Number of members in the term: 1 = a main effect, 2+ = an interaction. Derived, never stored"),
    .sm_col("genome_effect_terms", "contrast_signature",
            "The term's member contrasts in slot order, e.g. 'additive+dominance'"),
    .sm_col("genome_effect_terms", "family_key",
            "The fallback-family signature: trait, owner, and the ordered member states. Same key = the terms compete; different key = they sum"),
    .sm_col("genome_effect_terms", "scope_description",
            "Readable rendering of the term's origin rows, or 'common' when it has none"),
    # genome_effect_loci (view)
    .sm_tbl("genome_effect_loci",
            "View: one row per (term x locus), with locus_name joined from genome_meta. The place to answer which loci are causal for a trait; locus_name lives only here, so there is no id/name agreement invariant to keep in the base tables."),
    .sm_col("genome_effect_loci", "trait_name",
            "Genetic component trait; from genome_effects"),
    .sm_col("genome_effect_loci", "effect_owner",
            "Owning writer; from genome_effects"),
    .sm_col("genome_effect_loci", "id_genome_effect",
            "Term identifier; from genome_effect_members"),
    .sm_col("genome_effect_loci", "member_slot",
            "Position within the term; from genome_effect_members"),
    .sm_col("genome_effect_loci", "locus_id",
            "Locus identifier; from genome_effect_members"),
    .sm_col("genome_effect_loci", "locus_name",
...
.observation_model_descriptions <- function() {
  rbind(
    ...
    .sm_col("phenotype_meta", "condition_change_action",
            "Behavior when a stored residual of a correlated phenotype was drawn under a different residual condition level: 'error' (stop) or 'independent' (drop it from the conditioning set + warn); default 'error'"),
...
.results_descriptions <- function() {
  rbind(
    ...
    .sm_col("ind_tgv_total", "trait_name",
            "Genetic component trait name; from ind_tgv"),
    .sm_col("ind_tgv_total", "tgv_total",
            "Sum of tgv_value across all components for this individual and trait"),
    # ind_phenotype
    ...
    .sm_col("ind_phenotype", "liability_value",
            "Raw liability (mean + effects + TBV + residual) for categorical phenotypes with store_liability = TRUE; NULL otherwise"),
    .sm_col("ind_phenotype", "cat_name",
            "Category label for categorical phenotypes defined with cat_names; NULL otherwise"),
    .sm_col("ind_phenotype", "residual_value",
            "Realized liability-scale residual for model-generated and user_residual records; NULL for user_values and derived_formula records. Conditions later draws of correlated phenotypes"),
    .sm_col("ind_phenotype", "residual_condition_level",
            "condition_level of the residual (co)variance stratum the residual was drawn under; NULL when the unconditional R was used"),
    # ind_ebv
    .sm_tbl("ind_ebv",
