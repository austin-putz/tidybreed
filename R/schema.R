...
.genome_layer_descriptions <- function() {
  rbind(
    ...
    # trait_effect_cov
    .sm_tbl("trait_effect_cov",
            "Variance and covariance matrix storage for all random effects. One row per (effect_name, trait_name_1, trait_name_2); both (i,j) and (j,i) stored for off-diagonal entries."),
    .sm_col("trait_effect_cov", "id_trait_effect_cov",
            "Auto-incrementing primary key"),
    .sm_col("trait_effect_cov", "effect_name",
            "Effect identifier. Reserved: 'gen_add' (additive genetic G matrix), 'residual' (R matrix). Any other name maps to a user-defined random effect in trait_effects."),
    .sm_col("trait_effect_cov", "trait_name_1",
            "First trait in the (co)variance pair; FK to trait_meta.trait_name"),
    .sm_col("trait_effect_cov", "trait_name_2",
            "Second trait; equals trait_name_1 for diagonal (variance) entries"),
    .sm_col("trait_effect_cov", "cov_value",
    ...
    .sm_tbl("phenotype_residual_cov",
            "Residual (co)variance entries. One row per phenotype pair and optional condition. Supports unconditional, sex-stratified, and polynomial residual structures. Populated by define_phenotype() and add_residual_cov()."),
    .sm_col("phenotype_residual_cov", "id_residual_cov",
            "Auto-incrementing primary key"),
    .sm_col("phenotype_residual_cov", "phenotype_name_1",
            "First phenotype in the residual pair; FK to phenotype_meta.phenotype_name"),
    .sm_col("phenotype_residual_cov", "phenotype_name_2",
            "Second phenotype; equals phenotype_name_1 for diagonal (variance) entries"),
    .sm_col("phenotype_residual_cov", "cov_value",
            "Residual variance (diagonal) or covariance (off-diagonal)"),
    .sm_col("phenotype_residual_cov", "condition_column",
            "Column in condition_table that partitions residuals; NULL = unconditional"),
    .sm_col("phenotype_residual_cov", "condition_table",
            "Table containing condition_column; default 'ind_meta'"),
    .sm_col("phenotype_residual_cov", "condition_level",
            "Value of condition_column this row applies to (e.g. 'M', 'F')"),
    .sm_col("phenotype_residual_cov", "weight_type",
...
.trait_layer_descriptions <- function() {
  rbind(
    ...
    .sm_col("trait_meta", "trait_name",
            "Unique identifier used across genome_effects, ind_tbv, and trait_effect_cov"),
